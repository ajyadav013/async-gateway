...
# every file under `async_gateway/`. A source tree that is present but
...
# COPY. If `async_gateway` resolves anywhere but `site-packages`, this
...
resolved = pathlib.Path(async_gateway.__file__).resolve()
site_packages = pathlib.Path(sysconfig.get_paths()['purelib']).resolve()

if not resolved.is_relative_to(site_packages):
    raise SystemExit(
        'REFUSING TO RUN: async_gateway resolved to the source tree, not '
        'the installed wheel.\n'
...
print(f'async_gateway {async_gateway.__version__} from {resolved.parent}')
PY
