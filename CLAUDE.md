...
Configured by **claude-kit** with the SDLC profile **Enterprise — adds DevOps, Observability, full
audit + acceptance**. The agnostic pipeline rules apply unchanged; the conventions below make them
concrete for this stack.
...
- **Run: there is no run command.** This is a library — it has no entry point to start. Exercise it
  through the test suite or from a Python REPL, and treat any instruction to "start the server" or
...
- Build the distribution: `python -m build`. Note that `build` is **not** in
  `[project.optional-dependencies].dev`, so `pip install -e '.[dev]'` alone does not provide it —
...
Follow the protocol recipe in `.claude/rules/fastapi-patterns.md`
(protocol class → registry entry → envelope mapping → error mapping → tests → README section).
