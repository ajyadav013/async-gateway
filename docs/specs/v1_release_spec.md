...
- [ ] `client_keys=None` and `agent_path=None` are passed by default so ambient
      identities are never offered. Explicit key-based authentication is
      supported via `SFTPAuth` and the legacy `protocol_info['client_keys']`
      route. A test asserts both secure defaults and explicit forwarding.
...
| SFTP | `asyncssh.connect` patched with an async-CM double yielding a fake SFTP client | **`known_hosts` is not passed as `None`**, `client_keys=None` and `agent_path=None` by default, `connect_timeout`/`login_timeout` present, typed `SFTPAttrs` consumed, caller's `additional_arguments` unchanged |
...
| **R16** SFTP transport security | Omit `known_hosts` (system resolution); expose `known_hosts`/`host_key` for pinning; `insecure_skip_host_key_check` opt-out with a warning; `client_keys=None` and `agent_path=None` by default plus explicit key auth. | `logic/sftp_client.py` |
